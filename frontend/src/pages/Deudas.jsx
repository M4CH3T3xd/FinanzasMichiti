...
export default function Deudas() {
  ...
  const handleTogglePagado = async (deuda) => {
    try {
      await updateMut.mutateAsync({ id: deuda.id, pagado: !deuda.pagado })
    } catch {
  ...
  return (
    <div
      ...
      className={`flex items-center gap-3 p-3 rounded-xl border cursor-pointer transition-colors ${
        deuda.pagado
          ? 'bg-panel border-line opacity-50'
          : 'bg-panel border-line hover:border-brand-500/30'
      }`}
    >
      <span className="text-2xl shrink-0 w-9 text-center leading-none">{deuda.icono || '❓'}</span>
      ...
      <p className={`text-sm font-semibold shrink-0 ${
        deuda.tipo === 'debo' ? 'text-expense' : 'text-income'
      }`}>
        {fmt(deuda.monto)}
      </p>
      <button
        ...
        className={`w-7 h-7 rounded-full border-2 flex items-center justify-center shrink-0 transition-colors ${
          deuda.pagado
            ...
            : 'border-dim hover:border-income'
        }`}
      >
      ...
      <div className="bg-panel border border-line rounded-t-2xl sm:rounded-2xl w-full sm:max-w-md p-5 max-h-[92vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            ...
            <input
              type="text"
              placeholder="¿A quién le debes o te deben?"
              value={desc}
          ...
          <div>
            <label className="text-xs text-dim mb-1.5 block">
              Vencimiento <span className="opacity-50">(opcional)</span>
            </label>
            <input
              type="date"
              value={vencimiento}
          ...
          <div className="mt-3">
            {!confirmDel ? (
              ...
              <div className="bg-expense/10 border border-expense/30 rounded-xl p-3 text-center space-y-2">
                <p className="text-sm text-ink">¿Eliminar esta deuda?</p>
