...
import { getCategoryMeta, DEFAULT_CATEGORIES_EXPENSE, getCustomCategories } from '../lib/categoryMeta'
...
function PresupuestoModal({ presupuesto, categoriasUsadas, onSave, onDelete, onClose, saving, deleting }) {
  ...
  // Al editar la categoría ya está fija; al crear, excluir las que ya tienen presupuesto
  const catsDisponibles = presupuesto
    ? allCats
    : allCats.filter(c => !categoriasUsadas.includes(c))
      ...
      <div className="bg-panel border border-line rounded-t-2xl sm:rounded-2xl w-full sm:max-w-md p-5 max-h-[85vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            ...
            {presupuesto ? (
              // Editando: categoría fija
              <div className="flex items-center gap-2 px-3 py-2.5 bg-well border border-line rounded-xl">
                {(() => {
                  const meta = getCategoryMeta(presupuesto.categoria)
                  const Icon = meta.icon
                  return <Icon size={14} style={{ color: meta.color }} />
                })()}
                <span className="text-sm text-ink">{presupuesto.categoria}</span>
              </div>
            ) : (
              // Creando: selector
